...
export async function listApprovedCommunity(client: SupabaseClient, search = '') {
  const { data, error } = await client.rpc('admin_list_approved_community', {
    p_limit: 200,
    p_offset: 0,
  ...
  return {
    ...
    items: (value.items ?? []).map((item): ApprovedCommunityItem => ({
      accessId: String(item.access_id),
      userId: String(item.user_id),
      name: nullable(item.name),
      userStatus: String(item.user_status ?? 'ACTIVE'),
      platform: nullable(item.platform),
      username: nullable(item.username),
      phoneNumber: nullable(item.phone_number),
      platformUserId: nullable(item.platform_user_id),
      state: String(item.state ?? 'APPROVED'),
      inviteSentAt: nullable(item.invite_sent_at),
      joinRequestedAt: nullable(item.join_requested_at),
      activatedAt: nullable(item.activated_at),
      leftAt: nullable(item.left_at),
      removedAt: nullable(item.removed_at),
      lastVerifiedAt: nullable(item.last_verified_at),
      lastError: nullable(item.last_error),
      retryCount: Number(item.retry_count ?? 0),
      finalScore: item.final_score == null ? null : Number(item.final_score),
      recommendation: nullable(item.recommendation),
...
export async function listKnowledgeGaps(client: SupabaseClient, status = 'OPEN') {
  ...
  return {
    ...
    items: (value.items ?? []).map((item): KnowledgeGap => ({
      ...
      status: String(item.status ?? 'OPEN') as KnowledgeGap['status'],
...
export async function getSystemHealth(client: SupabaseClient): Promise<SystemHealth> {
  ...
  return {
    databaseStatus: String(value.database_status ?? 'UNKNOWN'),
    checkedAt: String(value.checked_at),
    platforms: Object.fromEntries(Object.entries(rawPlatforms).map(([key, item]) => [key, {
      lastInboundAt: nullable(item.last_inbound_at),
      messages24h: Number(item.messages_24h ?? 0),
    }])),
    outboxPending: Number(value.outbox_pending ?? 0),
    outboxDeadLetter: Number(value.outbox_dead_letter ?? 0),
    knowledgeFailed: Number(value.knowledge_failed ?? 0),
    knowledgeProcessing: Number(value.knowledge_processing ?? 0),
    aiUsageEvents: Number(value.ai_usage_events ?? 0),
    aiLastRecordedAt: nullable(value.ai_last_recorded_at),
    lastInboundAt: nullable(value.last_inbound_at),
    lastOutboundAt: nullable(value.last_outbound_at),
    lastKnowledgeUpdateAt: nullable(value.last_knowledge_update_at),
    n8nHealth: String(value.n8n_health ?? 'UNKNOWN'),
  };
}
...
export async function getActivityFeed(client: SupabaseClient, limit = 100): Promise<ActivityItem[]> {
  ...
  return (value.items ?? []).map((item) => ({
    ...
    entityId: nullable(item.entity_id),
...
export async function listAdminUsers(client: SupabaseClient): Promise<AdminUserItem[]> {
  ...
  return (value.items ?? []).map((item) => ({
    ...
    createdBy: nullable(item.created_by),
    isCurrentUser: item.is_current_user === true,
  }));
}
