...
export function ApprovedCommunityPage() {
  ...
  return <>
    ...
    <section className="table-card"><div className="table-scroll"><table><thead><tr><th>{tr('Member', 'العضو')}</th><th>{tr('Platform', 'المنصة')}</th><th>{tr('State', 'الحالة')}</th><th>{tr('Score', 'النتيجة')}</th><th>{tr('Activated', 'التفعيل')}</th><th>{tr('Last verified', 'آخر تحقق')}</th><th /></tr></thead><tbody>
      {items.map((item) => <tr key={item.accessId}><td><strong>{item.name || tr('Unnamed user', 'مستخدم بدون اسم')}</strong><small className="table-subtext">{item.username ? `@${item.username}` : item.phoneNumber || item.platformUserId || item.userId.slice(0, 8)}</small></td><td>{item.platform ? <span className={`platform ${item.platform}`}>{item.platform}</span> : '—'}</td><td><Status value={item.state} />{item.lastError && <small className="table-subtext error-text">{item.lastError}</small>}</td><td>{item.finalScore == null ? '—' : `${Math.round(item.finalScore)}/100`}</td><td>{date(item.activatedAt)}</td><td>{date(item.lastVerifiedAt)}</td><td className="table-action"><Link className="row-link" to={`/users/${item.userId}`}>{tr('Open user', 'فتح المستخدم')} →</Link></td></tr>)}
...
export function KnowledgeGapsPage() {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">{tr('Knowledge improvement', 'تحسين المعرفة')}</p><h1>{tr('Knowledge gaps', 'فجوات المعرفة')}</h1><p className="muted page-subtitle">{tr('Questions the assistant could not answer from verified knowledge.', 'الأسئلة التي لم يتمكن المساعد من الإجابة عنها من المعرفة الموثقة.')}</p></div><select className="compact-select" value={status} onChange={(e) => setStatus(e.target.value)}><option value="OPEN">{tr('Open', 'مفتوح')}</option><option value="RESOLVED">{tr('Resolved', 'تم الحل')}</option><option value="IGNORED">{tr('Ignored', 'متجاهل')}</option><option value="ALL">{tr('All', 'الكل')}</option></select></header>
    {error && <p className="form-error">{error}</p>}
    <section className="table-card"><div className="table-scroll"><table><thead><tr><th>{tr('Question', 'السؤال')}</th><th>{tr('Asked', 'عدد المرات')}</th><th>{tr('Platform', 'المنصة')}</th><th>{tr('Last seen', 'آخر ظهور')}</th><th>{tr('Status', 'الحالة')}</th><th /></tr></thead><tbody>
      {items.map((gap) => <tr key={gap.id}><td className="question-cell"><strong>{gap.question}</strong><small className="table-subtext">{gap.language?.toUpperCase() || '—'}</small></td><td><strong>{gap.occurrenceCount}</strong></td><td>{gap.platform || '—'}</td><td>{date(gap.lastSeenAt)}</td><td><Status value={gap.status} /></td><td className="table-action"><div className="decision-actions">{gap.status !== 'RESOLVED' && <button className="compact-button" onClick={() => void setGap(gap, 'RESOLVED')}>{tr('Resolve', 'حل')}</button>}{gap.status !== 'IGNORED' && <button className="compact-button" onClick={() => void setGap(gap, 'IGNORED')}>{tr('Ignore', 'تجاهل')}</button>}{gap.status !== 'OPEN' && <button className="compact-button" onClick={() => void setGap(gap, 'OPEN')}>{tr('Reopen', 'إعادة فتح')}</button>}</div></td></tr>)}
...
export function AdminUsersPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">{tr('Administration', 'الإدارة')}</p><h1>{tr('Admin users', 'المشرفون')}</h1><p className="muted page-subtitle">{tr('Accounts currently registered for administrator access.', 'الحسابات المسجلة حالياً للوصول الإداري.')}</p></div><span className="status-pill neutral">{items.length}</span></header><section className="table-card"><div className="table-scroll"><table><thead><tr><th>{tr('Administrator', 'المشرف')}</th><th>{tr('Status', 'الحالة')}</th><th>{tr('Added', 'أضيف')}</th><th>{tr('User ID', 'معرف المستخدم')}</th></tr></thead><tbody>{items.map((item) => <tr key={item.userId}><td><strong>{item.email || tr('Email unavailable', 'البريد غير متاح')}</strong>{item.isCurrentUser && <small className="table-subtext">{tr('Current account', 'الحساب الحالي')}</small>}</td><td><Status value={item.isActive ? 'ACTIVE' : 'DISABLED'} /></td><td>{date(item.createdAt)}</td><td className="mono">{item.userId}</td></tr>)}</tbody></table></div></section></>;
}
