    ...
    getKnowledgeDocument(getSupabaseClient(), documentId)
      .then((value) => {
        ...
        const fallbackText = value.content?.trim() || value.chunks
          ...
          .join('\n\n');
        const html = value.editorHtml?.trim() || plainTextToHtml(fallbackText);
      ...
      <div className="document-editor-shell">
        ...
        <div className="document-editor-ribbon" aria-label={tr('Formatting toolbar', 'شريط التنسيق')}>
          <div className="ribbon-group">
            <button type="button" title={tr('Undo', 'تراجع')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('undo')}>↶</button>
            <button type="button" title={tr('Redo', 'إعادة')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('redo')}>↷</button>
          </div>
          <div className="ribbon-group">
            <button type="button" className="ribbon-text-button" onMouseDown={toolbarMouseDown} onClick={() => runCommand('formatBlock', 'p')}>{tr('Normal', 'عادي')}</button>
            <button type="button" className="ribbon-text-button" onMouseDown={toolbarMouseDown} onClick={() => runCommand('formatBlock', 'h1')}>{tr('Title', 'عنوان')}</button>
            <button type="button" className="ribbon-text-button" onMouseDown={toolbarMouseDown} onClick={() => runCommand('formatBlock', 'h2')}>{tr('Heading', 'رأس')}</button>
          </div>
          <div className="ribbon-group">
            <button type="button" className="format-bold" onMouseDown={toolbarMouseDown} onClick={() => runCommand('bold')}>B</button>
            <button type="button" className="format-italic" onMouseDown={toolbarMouseDown} onClick={() => runCommand('italic')}>I</button>
            <button type="button" className="format-underline" onMouseDown={toolbarMouseDown} onClick={() => runCommand('underline')}>U</button>
          </div>
          <div className="ribbon-group">
            <button type="button" title={tr('Bulleted list', 'قائمة نقطية')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('insertUnorderedList')}>• List</button>
            <button type="button" title={tr('Numbered list', 'قائمة مرقمة')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('insertOrderedList')}>1. List</button>
          </div>
          <div className="ribbon-group">
            <button type="button" title={tr('Align left', 'محاذاة لليسار')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('justifyLeft')}>≡</button>
            <button type="button" title={tr('Align center', 'توسيط')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('justifyCenter')}>≣</button>
            <button type="button" title={tr('Align right', 'محاذاة لليمين')} onMouseDown={toolbarMouseDown} onClick={() => runCommand('justifyRight')}>≡</button>
          </div>
          <div className="ribbon-group">
            <button type="button" className="ribbon-text-button" onMouseDown={toolbarMouseDown} onClick={() => runCommand('removeFormat')}>{tr('Clear formatting', 'مسح التنسيق')}</button>
          </div>
        </div>
