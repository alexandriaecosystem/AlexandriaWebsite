...
describe('analytics and knowledge RPC boundary', () => {
  it('normalizes dashboard metrics', async () => {
    const rpc = vi.fn().mockResolvedValue({ data: {
      total_users: 4, active_users: 4, approved_users: 1, pending_reviews: 2, blocked_users: 0,
    ...
    expect(result).toMatchObject({ totalUsers: 4, totalMessages: 45, aiCostTotal: 0.0021, failedOperations: 3 });
